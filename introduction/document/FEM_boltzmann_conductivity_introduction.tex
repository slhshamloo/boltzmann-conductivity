...
\begin{gather}
    ...
    \pdv{f}{t} + \frac{\vb{p}}{m}\vdot\grad f + \vb{F}\vdot\grad_{\vb{p}}f = I[f],
\end{gather}
...
\begin{equation}
    \pdv{f}{t} + \frac{\hbar\vb{k}}{m}\vdot\grad f
...
\end{equation}
(we assume the material is uniform), and using
\begin{equation}
    \vb{v} = \frac{\grad_{\vb{k}}\varepsilon}{\hbar}
\end{equation}
where $\vb{v}$ is the group velocity $\varepsilon$ is the energy, we get
\begin{equation}
    \pdv{g}{t} + \frac{\hbar\vb{k}}{m}\vdot\grad g
...
uniform fields, so we can drop the spatial gradient term. Also, for the conductivity tensor, we
are only interested in the linear response to the electric field, so we can drop the electric field
term on the right hand side. Finally, linearizing the collision integral in terms of the fluctuation
$g$, we have
\begin{equation}
...
\sloppy{In-scattering is dropped in the \emph{relaxation time
approximation}, which is the case for the Chambers formula. Our goal here is to keep this term, as
it especially becomes important when the Fermi surface has strong anisotropy, which is the case for
cuprates. When the Fermi surface is roughly isotropic, $C(\vb{k}, \vb{k'})$ does not strongly depend
on $\vb{k}$ and, because of charge conservation,
\begin{equation}
...
Chambers formula is just wrong for anisotropic Fermi surfaces. So, why does it work effectively
...
\begin{equation}
    (Af)_i = \int\dd{\vb{k}} \psi_i(\vb{k}) (Af)(\vb{k}) = \sum_j A^i_jf^j
    \int\dd{\vb{k}} \psi_i(\vb{k})\psi_j(\vb{k}) = \sum_j M_{ij} A^i_j f^j.
    \label{eq:integral}
\end{equation}
...
\begin{equation}
    \int \dd{\vb{k}} \psi_i(\vb{k}) (Af)(\vb{k}) = \int \dd{\vb{k}} \psi_i(\vb{k}) h(\vb{k}).
\end{equation}
...
For quasi-2D materials, it is more efficent to calculate the conductivity layer by layer in 2D and
...
For the out-scattering term $\Gamma_{ij}$, we first need to expand the $k$-dependant terms.
This gives
\begin{equation}
...
Finally, for the in-scattering term $S_{ij}$, we must expand the $k$-dependant terms again.
