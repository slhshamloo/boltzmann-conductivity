...
\begin{equation}
    \frac{1}{\tau_j} = \sum_i \int\dd{\vb{k}} C_{ij} \phi_i(\vb{k}).
\end{equation}
For the 3D case with triangular elements, this means
\begin{equation}
    \frac{1}{\tau_j} = \sum_i \alpha_i C_{ij}
\end{equation}
...
the symmetry basis indices is not arbitrary any more. This time, strictly
\begin{equation}
...
FEM basis, and keep only the largest singular values. This can be achieved efficiently without
constructing the full matrix and only computing the action of the kernel on vectors. Then, using
methods like the Lanczos algorithm, we can compute the largest singular values and the corresponding
singular vectors, without solving the full eigendecomposition problem.
