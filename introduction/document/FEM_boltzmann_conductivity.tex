...
\end{equation} 
where $C(\vb{k}, \vb{k'})$ is a scattering kernel. We call the two terms the \emph{in-scattering} and
...
Mathematically, the scattering kernel $C(\vb{k}, \vb{k'})$ can be any arbitrary function. However,
without any other constraints, it creates a global interaction between the states, which kills the
performance. The other operators only involve interactions between neighboring elements, which
produces sparse matrices, with elements only on a few diagonals. This is what makes the this method
computationally efficient. Including the scattering kernel breaks this sparsity by having
interactions between arbitrary pairs of points, and nonzero elements in any position of
the matrices. Going to the basis based on the symmetries of the system, and then keeping a limited
number of coefficients in the expansion, helps maintain the performance by reducing the rank of the
matrices we have to deal with.
