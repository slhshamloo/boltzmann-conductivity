...
\usepackage{amsmath,amssymb,bbold,bm,csquotes,empheq,graphicx,hyperref,siunitx,subcaption,upgreek,xcolor}
...
\title{FEM for Boltzmann Conductivity}
\author{Saleh Shamloo, Chaitanya Murthy}
...
\end{equation} 
where $C(\vb{k}, \vb{k'})$ is the \emph{scattering kernel}. We call the two terms the
\emph{in-scattering} and \emph{out-scattering} terms respectively. The out-scattering is usually
expressed with the \emph{relaxation time} $\tau$ or \emph{scattering rate} $\Gamma=1/\tau$, which
is equal to
\begin{equation}
...
completeness, We will go with the latter case (just setting $\omega=0$ and using the regular,
...
\subsection{The Collision Integral}
\subsubsection{The Relaxation Time Approximation (RTA)}
\sloppy{The in-scattering term of the collision integral
\begin{equation}
    \int\dd{\vb{k'}} C(\vb{k}, \vb{k'}) g(\vb{k'}; t)
\end{equation}
is dropped in the \emph{relaxation time approximation}, which is the case for
...
Due to detailed balance (discussed in the next paragraph), ignoring inelastic scattering,
$C(\vb{k}, \vb{k'}) = C(\vb{k'}, \vb{k})$, so if we consider any anisotropy in the out-scattering,
we must do the same for the in-scattering. This means the Chambers formula is just wrong for
anisotropic scattering. So, why does it work effectively anyway? The scattering in cuprates
might only be low-angle, so the in-scattering is small compared to the out-scattering.}

\subsubsection{Detailed Balance}
Another important point to consider is that this linearization implicitly assumes \emph{detailed
balance}, meaning that $f_0$ is in equilibrium. Mathematically, it can be expressed as
\begin{equation}
    I[f_0] = 0 \implies \int\dd{\vb{k'}} C(\vb{k}, \vb{k'}) f_0(\vb{k'})
    = \int\dd{\vb{k'}} C(\vb{k'}, \vb{k}) f_0(\vb{k}).
\end{equation}
This will be later used to impose constraints on the scattering kernel. Note that detailed balance
only ensures that $f_0$ is the equilibrium distribution, and it has nothing to do with
charge conservation.

\subsubsection{Charge Conservation}
Charge conservation can be expressed as
\begin{equation}
    \dv{n}{t} = \dv{t} \int\dd{\vb{k}} f(\vb{k}) = 0.
\end{equation}
In our finite-element treatment of the problem, the integration domain is fixed. This is admittedly
a simplifying assumption. For example, think of a Fermi surface as the integration domain. If we
allow the chemical potential to change, then the Fermi surface changes as well, so the integration
domain is not fixed. Other alternative formulations with variable domains can exist, but it does not
concern our current formulation. Now, with a fixed integration domain, we can pull the time
derivative inside the integral, and from the Boltzmann equation, we have
\begin{equation}
    \int\dd{\vb{k}} \dv{f}{t} = \int\dd{\vb{k}} I[f] = 0.
\end{equation}
This means, for all functions $f$,
\begin{equation}
    \int\dd{\vb{k}} \int\dd{\vb{k'}} C(\vb{k}, \vb{k'}) f(\vb{k'})
    = \int\dd{\vb{k}} \int\dd{\vb{k'}} C(\vb{k'}, \vb{k}) f(\vb{k}).
\end{equation}
But, if you rename $\vb{k}$ and $\vb{k'}$ and swap the dummy integration variables, you can get from
the expression of one side to the other, and this is therefore identically true. The linearization
we chose for the collision integral is automatically charge-conserving, and no additional
...
think of it as a syntactic game. We will also not use the Einstein summation convention, just to
make things completely explicit and hopefully clearer.

Defining the basis as the set $\{\phi_i(\vb{k})\}$, we can approximate any function by a linear
combination of these basis functions;
\begin{equation}
    f(\vb{k}) = \sum_i f^{i}\phi_i(\vb{k}). \label{eq:vector}
\end{equation}
...
\begin{equation}
    f_i \equiv \int\dd{\vb{k}} \phi_i(\vb{k}) f(\vb{k})
    = \sum_j f^j \int\dd{\vb{k}} \phi_i(\vb{k}) \phi_j(\vb{k})
    \label{eq:integral_vector}
\end{equation}
It is useful to define the quantity
\begin{equation}
    M_{ij} \equiv \int \dd{\vb{k}} \phi_i(\vb{k}) \phi_j(\vb{k}),
    \label{eq:overlap}
\end{equation}
...
\begin{equation}
    (Af)(\vb{k}) = \sum_i (Af)^i \phi_i(\vb{k}) \equiv \sum_{ij} A^i_j f^j \phi_i(\vb{k}).
\end{equation}
For the integrals, we have
\begin{equation}
    (Af)_i = \int\dd{\vb{k}} \phi_i(\vb{k}) (Af)(\vb{k}) = \sum_{jk} A^j_kf^k
    \int\dd{\vb{k}} \phi_i(\vb{k})\phi_j(\vb{k}) = \sum_{jk} M_{ij} A^j_k f^k.
    \label{eq:integral}
\end{equation}
...
we get
\begin{equation}
    \sum_j A_{ij} f^j = \int\dd{\vb{k}} \phi_i(\vb{k}) \int\dd{\vb{k'}} A(\vb{k}, \vb{k'}) f(\vb{k'}).
\end{equation}
expanding the function $f(\vb{k'})$ in terms of the basis functions from equation \eqref{eq:vector},
\begin{equation}
    \sum_j A_{ij} f^j = \int\dd{\vb{k}} \phi_i(\vb{k}) \int\dd{\vb{k'}} A(\vb{k}, \vb{k'})
    \sum_j f^j \phi_j(\vb{k'}).
\end{equation}
This is true for any function vector $f^j$, so we can drop it from both sides. This finally gives
\begin{equation}
    A_{ij} = \int\dd{\vb{k}} \int\dd{\vb{k'}} \phi_i(\vb{k}) A(\vb{k}, \vb{k'}) \phi_j(\vb{k'}).
...
$\phi_i(\vb{k})$ and integrate over $\vb{k}$. This gives
\begin{equation}
    \int \dd{\vb{k}} \phi_i(\vb{k}) (Ag)(\vb{k}) = \int \dd{\vb{k}} \phi_i(\vb{k}) h(\vb{k}).
\end{equation}
...
\begin{equation}
    f(\vb{k})g(\vb{k}) = \sum_{ij} f^i g^j \phi_i(\vb{k}) \phi_j(\vb{k}),
\end{equation}
and the integral over this product is
\begin{equation}
    \int\dd{\vb{k}} f(\vb{k}) g(\vb{k}) = \sum_{ij} f^i g^j \int\dd{\vb{k}}
    \phi_i(\vb{k})\phi_j(\vb{k}) = \sum_{ij} f^i g^j M_{ij} = \sum_i f^i g_i.
\end{equation}
...
\begin{equation}
    ...
    \frac{\phi_i(\vb{k})\phi_j(\vb{k})\phi_k(\vb{k})}{v_k}.
\end{equation}
...
\begin{empheq}[left={\Gamma_{ij}=\sum_k\Pi_{ijk}\gamma^k\implies\empheqlbrace}]{align}
    \Pi_{ijk} &= \int\dd{\vb{k}} \phi_i(\vb{k})\phi_j(\vb{k})\phi_k(\vb{k}), \\
...
gives
\begin{equation}
    S_{ij} = -\sum_{mn} \int\dd{\vb{k}} \int\dd{\vb{k'}} \phi_i(\vb{k})\phi_m(\vb{k})
    \frac{C_{mn}}{v_m}\phi_n(\vb{k'})\phi_j(\vb{k'}).
\end{equation}
...
\begin{empheq}[left={S_{ij}=\sum_{m,n}\Xi_{ijmn}s^{mn}\implies\empheqlbrace}]{align}
    \Xi_{ijmn} &= \int\dd{\vb{k}} \int\dd{\vb{k'}} \phi_i(\vb{k})\phi_j(\vb{k'})
    \phi_m(\vb{k})\phi_n(\vb{k'}), \\
    s^{mn} &= -\frac{C_{mn}}{v_m}.
\end{empheq}
...
\begin{empheq}[left={\phi_i(\vb{k})=\empheqlbrace}]{align}
    &x,& &\vb{k} \in I_{i-1}; \\
    &1-x,& &\vb{k} \in I_i; \\
    &0,& &\vb{k} \in I_j,\ j\notin \{i-1,i\}.
\end{empheq}
...
\begin{equation}
\begin{aligned}
    M_{ii} &= \int\dd{k}\phi_i(\vb{k})\phi_i(\vb{k}) = \int_{I_{i-1}} \dd{\vb{k}}
    \qty[\phi_i(\vb{k})]^2 + \int_{I_i} \dd{\vb{k}} \qty[\phi_i(\vb{k})]^2\\
    &= \int_0^1 \dd{x} x^2 \ell_{i-1} + \int_0^1 \dd{x} (1-x)^2 \ell_i
    = \frac{\ell_{i-1} + \ell_i}{3},
\end{aligned}
\end{equation}
\begin{equation}
    M_{i,i+1} = M_{i+1,i} = \int_{I_i} \dd{\vb{k}} \phi_i(\vb{k})\phi_{i+1}(\vb{k})
...
direction of $\vb{k}_{i-1}$ to $\vb{k}_i$, the function $\phi_i$ increases on $I_{i-1}$, and in the
...
\begin{equation}
    \Pi_{iii} = \int_{I_i} \dd{\vb{k}} \qty[\phi_i(\vb{k})]^3 + \int_{I_{i-1}}
        \dd{\vb{k}} \qty[\phi_i(\vb{k})]^3
...
\begin{equation}
    ...
    \qty[\phi_i(\vb{k})]^2\phi_{i+1}(\vb{k}) = \int_0^1 \dd{x} (1-x)^2 x \ell_i = \frac{\ell_i}{12},
\end{equation}
\begin{equation}
    \Pi_{i,i+1,i+1} = \Pi_{i+1,i,i+1} = \Pi_{i+1,i+1,i} = \int_{I_i} \dd{\vb{k}}
    \phi_i(\vb{k})\qty[\phi_{i+1}(\vb{k})]^2 = \int_0^1 \dd{x} x(1-x)^2 \ell_i = \frac{\ell_i}{12},
\end{equation}
\begin{equation}
...
$\phi_i$ peaks with value 1 at the point $\vb{k}_i$, is linear on $T_i$, and is 0 everywhere else.
...
\begin{empheq}[left={\phi_i(\vb{k})=\empheqlbrace}]{align}
    &1 - x - y,& &\vb{k} \in T_i, (x, y)_{\max(\phi_i)} = (0, 0); \\
    &x,& &\vb{k} \in T_i, (x, y)_{\max(\phi_i)} = (1, 0); \\
    &y,& &\vb{k} \in T_i, (x, y)_{\max(\phi_i)} = (0, 1); \\
    &0,& &\vb{k} \notin T_i.
\end{empheq}
...
\begin{align}
M_{ii} &= \int\dd{\vb{k}}\phi_i(\vb{k})\phi_i(\vb{k})
    = \sum_{(i,j,k) \in T_i} \int_{T(i,j,k)} \dd{\vb{k}} (\phi_i(\vb{k}))^2 \\
    &= \sum_{(i,j,k) \in T_i} \alpha(i,j,k) \int_T \dd{x} \dd{y} (1 - x - y)^2
    = \frac{1}{12} \sum_{(i,j,k) \in T_i} \alpha(i,j,k), \notag \\
M_{ij} &= \int\dd{\vb{k}}\phi_i(\vb{k})\phi_j(\vb{k})
    = \sum_{(i,j,k) \in T_i \cap T_j} \int_{T(i,j,k)} \dd{\vb{k}} \phi_i(\vb{k}) \phi_j(\vb{k}) \\
...
\end{align}
\begin{align}
\Pi_{iii} &= \int\dd{\vb{k}}\phi_i(\vb{k})\phi_i(\vb{k})\phi_i(\vb{k})
    = \sum_{(i,j,k) \in T_i} \int_{T(i,j,k)} \dd{\vb{k}} (\phi_i(\vb{k}))^3 \\
    &= \sum_{(i,j,k) \in T_i} \alpha(i,j,k) \int_T \dd{x} \dd{y} (1 - x - y)^3
    = \frac{1}{20} \sum_{(i,j,k) \in T_i} \alpha(i,j,k), \notag \\
\Pi_{iij} &= \Pi_{iji} = \Pi_{jii} = \int\dd{\vb{k}}\phi_i(\vb{k})\phi_i(\vb{k})\phi_j(\vb{k})
    = \sum_{(i,j,k) \in T_i \cap T_j} \int_{T(i,j,k)} \dd{\vb{k}}
        (\phi_i(\vb{k}))^2 \phi_j(\vb{k}) \\
    &= \sum_{(i,j,k) \in T_i \cap T_j} \alpha(i,j,k) \int_T \dd{x} \dd{y} (1 - x - y)^2 x
    = \frac{1}{60} \sum_{(i,j,k) \in T_i \cap T_j} \alpha(i,j,k), \notag\\
\Pi_{ijk} &= \int_{T(i,j,k)} \dd{\vb{k}}\, \phi_i(\vb{k}) \phi_j(\vb{k}) \phi_k(\vb{k})
...
$\phi_j$ on triangle $T(i,j,k)$. Since $\phi_j$ is linear on this triangle and peaks at $j$,
its gradient is a constant vector pointing to $j$ and perpendicular to the opposite edge
$\vb{k}_k - \vb{k}_i$.
\begin{equation}
\grad_{\vb{k}} \phi_j(\vb{k}) = \frac{\vb{u}_j(i,j,k)}{u^2_j(i,j,k)};
\end{equation}
...
\begin{equation}
    \qty(\hat{\vb{v}}(\vb{k})\cross\vu{B})\vdot\grad_{\vb{k}}\phi_j
    = \qty(\grad_{\vb{k}}\phi_j\cross\hat{\vb{v}}(\vb{k}))\vdot\vu{B}
...
\begin{align}
    (D_{\vu{v}\cross\vu{B}})_{ij} &= \int\dd{\vb{k}}\phi_i(\vb{k})
    \qty(\vu{v}(\vb{k})\cross\vu{B})\vdot\grad_{\vb{k}}\phi_j(\vb{k}) \\
...
\begin{gather}
\Xi_{ijmn} = \qty(\int\dd{\vb{k}}\phi_i(\vb{k})\phi_m(\vb{k}))
    \qty(\int\dd{\vb{k'}}\phi_j(\vb{k'})\phi_n(\vb{k'})) = M_{im} M_{jn} \\
...
\begin{gather}
    ...
    \begin{aligned}
    S_{ij} = \frac{1}{144}&\left(\frac{1}{4}\sum_{m,n} \alpha_{im} \alpha_{jn}
        s^{mn} \delta_{\expval{im}}\delta_{\expval{jn}}
        + \alpha_i\alpha_j s^{ij} \right.\\
        &\quad+ \left.\frac{1}{2}\sum_n \alpha_i\alpha_{jn} s^{in} \delta_{\expval{jn}}
        + \frac{1}{2}\sum_m \alpha_{im}\alpha_j s^{mj} \delta_{\expval{im}}\right).
    \end{aligned}
\end{gather}
$\delta_{\expval{ij}}$ is equal to 1 if $i$ and $j$ are neighboring points, and 0 otherwise.
Technically, due to the definitions, it is not really needed, but we include it to make things
a bit clearer.

\section{Scattering Kernel}
The FEM allows us to use any arbitrary scattering kernel $C(\vb{k}, \vb{k'})$. However, there
is still the question of how we should build it to have a useful model. A physically relevant
approach is to expand the scattering kernel in terms of basis functions that respect the
symmetries of the system. That is,
\begin{equation}
    C(\vb{k}, \vb{k'}) = \sum_{a,b} \tilde{C}_{ab} \psi_a(\vb{k})\psi_b^*(\vb{k'}),
    \label{eq:kernel_expansions}
\end{equation}
where $\psi_a$ and $\psi_b$ are basis functions based on the symmetries of the system. For
example, for a system with translational and rotational invariance, one can use spherical
harmonics as the basis functions.

Before going further, note that due to detailed balance, over a single Fermi surface (i.e. no
inelastic scattering), we have
\begin{equation}
    C(\vb{k}, \vb{k'}) = C(\vb{k'}, \vb{k}).
\end{equation}
Which, together with the reality of the scattering kernel, imposes
\begin{equation}
    \tilde{C}_{ab} = \tilde{C}_{ba}^*.
\end{equation}

Mathematically, the scattering kernel $C(\vb{k}, \vb{k'})$ can be any arbitrary function (that also
satisfies the detailed balance condition). However, without any other constraints, it creates a
global interaction between the states, which kills the performance. The other operators only involve
interactions between neighboring elements, which produces sparse matrices, with elements only on a
few diagonals. This is what makes the this method computationally efficient. Including the
scattering kernel breaks this sparsity by having interactions between arbitrary pairs of points, and
nonzero elements in any position of the matrices. Going to the basis based on the symmetries of the
system, and then keeping a limited number of coefficients in the expansion, helps maintain the
performance by reducing the rank of the matrices we have to deal with.

\subsection{Change of Basis}
In the symmetry basis, a function $f(\vb{k})$ can be expressed as
\begin{equation}
    f(\vb{k}) = \sum_a \tilde{f}^a \psi_a(\vb{k})
\end{equation}
(the same indexing notation with FEM is used to avoid confusion). From equation \eqref{eq:vector},
\begin{equation}
    \sum_i f^i \phi_i(\vb{k}) = \sum_a \tilde{f}^a \psi_a(\vb{k}).
    \label{eq:vector_basis_expansion}
\end{equation}
To get the coefficients $\tilde{f}^a$ in the symmetry basis, we take the inner product of both
sides with $\psi_a$. The symmetry basis is orthonormal, so we have
\begin{equation}
    \tilde{f}^a = \sum_{i} f^i \int\dd{\vb{k}}\phi_i(\vb{k}) \psi_a^*(\vb{k}).
\end{equation}
In matrix form, this is
\begin{equation}
    \tilde{f}^i = \sum_j U^i_j f^j; \quad U^i_j = \int\dd{\vb{k}}\psi_i^*(\vb{k})\phi_j(\vb{k}),
\end{equation}
where $U$ is the change of basis matrix from the FEM basis to the symmetry basis.

To actually calculate the change of basis matrix, we can either numerically integrate the symmetry
basis functions over the FEM elements (with a method like Gaussian quadrature), or do it
analytically. In 3D, the change of basis matrix is calculated as
\begin{align}
    U^i_j &= \sum_{(j,k,l) \in T_j} \int_{T(j,k,l)} \dd{\vb{k}} \psi_i^*(\vb{k}) \phi_j(\vb{k})
    = \sum_{(j,k,l) \in T_j} \alpha(j,k,l) \int_T \dd{x} \dd{y} \psi_i^*(\vb{k}(x,y))(1-x-y)
    \\ &= \sum_{(j,k,l) \in T_j} \alpha(j,k,l) \int_T \dd{x} \dd{y}
    \psi_i^*(\vb{k}_j(1-x-y) + \vb{k}_k x + \vb{k}_l y)(1-x-y)
\end{align}

Before moving on to change of basis of the in-scattering matrix, we should familiarize ourselves
with the positioning of the indices. In the symmetry basis, the overlap matrix is the identity
matrix, because the basis functions are orthonormal. This means we can freely raise and lower
indices related to the symmetry basis. However, for the change of basis matrix, we need to be
careful with the ordering of the indices. The change of basis $U$ takes us from the FEM basis
to the symmetry basis, so its first index, which is not summed over, should always be the
symmetry basis index.

Now, we rewrite the in-scattering term in the symmetry basis. From equation
\eqref{eq:kernel_expansions},
\begin{equation}
    S_{ij} = -\sum_{a,b} \int\dd{\vb{k}} \int\dd{\vb{k'}}\phi_i(\vb{k})\psi_a(\vb{k})
    \frac{\tilde{C}_{ab}}{\tilde{v}_a}\psi_b^*(\vb{k'})\phi_j(\vb{k'})
    \label{eq:in_scattering_symmetry}
\end{equation}
Using the change of basis matrix, and carefully arranging the indices for correct contraction,
this can be rewritten as
\begin{equation}
    S_{ij} = \sum_{a,b} U^*_{ai} \tilde{S}^a_b U^b_j
    = \sum_{a,b} U^\dagger_{ia} \tilde{S}^a_b U^b_j,
\end{equation}
which is precisely the change of basis formula for the matrix $S$ from the symmetry basis to
the FEM basis.

\subsection{Sherman--Morrison--Woodbury Formula}
As discussed earlier, the in-scattering term breaks the sparsity of the differential matrix $A$.
However, the in-scattering matrix is actually of a low-rank. We can exploit this fact to solve
the matrix equations much more efficiently. If we split the differential matrix into a sparse
part $A_0$ and a low-rank part $U^\dagger \tilde{S} U$, then the Sherman--Morrison--Woodbury
formula for the inverse gives
\begin{equation}
    A^{-1} = (A_0 + U^\dagger \tilde{S} U)^{-1}
    = A_0^{-1} - A_0^{-1} U^\dagger (\tilde{S}^{-1} + U A_0^{-1} U^\dagger)^{-1} U A_0^{-1}.
\end{equation}
If there are $N_{\text{sym}}$ symmetry basis functions, then $A_0^{-1} U^{\dagger}$ is
$N_{\text{sym}}$ sparse inversions, and $\tilde{S}^{-1}$ and
$(\tilde{S}^{-1} + U A_0^{-1} U^\dagger)^{-1}$ are two $N_{\text{sym}}\times N_{\text{sym}}$
dense matrix inversions. So, if $N_{\text{sym}}$ is small, the inversion will stay
computationally efficient. If we choose an apropriate symmetry basis, then the number of
physically relevant terms in the expansion of the scattering kernel should be limited, and
$N_{\text{sym}}$ should indeed be small.


\subsection{Out-scattering Term}
When calculating the out-scattering term from the scattering kernel instead of using an external
hypothesis, we need to integrate it over the Fermi surface (equation \eqref{eq:tau_kernel}).
In order to do this, one can leverage the same basis functions, and split the integration into
multiple integrals over different elements. So, the equation becomes
\begin{equation}
    \sum_j\frac{1}{\tau_j}\phi_j(\vb{k}) =
    \sum_{i,j} \int\dd{\vb{k'}} C_{ij} \phi_i(\vb{k'})\phi_j(\vb{k})
\end{equation}
For the 3D case with triangular elements, this means
\begin{equation}
    \frac{1}{\tau_j} = \sum_{i}C_{ij} \int_{T_i} \dd{x}\dd{y} (1-x-y)
    = \sum_{i}\frac{C_{ij}\alpha_i}{6}.
    \label{eq:tau_kernel_fem}
\end{equation}
The problem with this approach is that the approximation is too rough. This is equivalent to
assuming the kernel is constant over each element. A more accurate approach would be to do a more
sophisticated numerical integral of the kernel over each element (e.g., Gaussian quadrature).
In the 3D case, this gives
\begin{equation}
    \frac{1}{\tau_j} = \sum_{i}\int_{T_i} \dd{x}\dd{y} C(\vb{k}(x,y), \vb{k}_j).
\end{equation}
This is more accurate, but very computationally expensive. There are as many integrals to do
as the number of triangles, for \emph{each} point on the Fermi surface.
This scales as $\mathcal{O}(N^2)$, which is even larger than the $\mathcal{O}(N^{3/2})$ of the
sparse matrix inversion, as discussed in the next section.

Again, by moving to the symmetry basis, we can significantly reduce the computational cost.
In the symmetry basis, the (out-)scattering rate is given by
\begin{empheq}[left=\empheqlbrace]{align}
    \frac{1}{\tau(\vb{k})} &= \sum_{b}\frac{1}{\tilde{\tau_b}}\psi_b(\vb{k}), \\
    \frac{1}{\tau(\vb{k})} &=
    \sum_{a,b}\int\dd{\vb{k'}} \tilde{C}_{ab} \psi_a(\vb{k'})\psi_b^*(\vb{k}).
\end{empheq}
Note that functions can also be expanded in terms of the complex conjugation of the
basis functions. Assuming the reality condition $\tau^*(\vb{k}) = \tau(\vb{k})$ (which is satisfied
by the Hermicitiy of $\tilde{C}_{ab}$), this gives
\begin{equation}
    \frac{1}{\tilde{\tau_b}} = \sum_a \int\dd{\vb{k}} \tilde{C}_{ab}^* \psi_a^*(\vb{k}).
\end{equation}
This gives us $N_{\text{sym}}\times N_{\text{elements}}$ integrals to compute. If $N_{\text{sym}}$
is small (as it should be), this is much more efficient than the $\mathcal{O}(N^2)$ integrals we had
to compute in the FEM basis. then after computing the integrals, we can go back to the FEM basis by
applying a change of basis. Using the FEM to symmetry change of basis matrix $U$, this gives
\begin{equation}
    \frac{1}{\tau_j} = \qty(\frac{1}{\tau})^j
    = \sum_b\qty(U^{-1})^j_b \qty(\frac{1}{\tilde{\tau}})^b.
\end{equation}
We need to be careful with the matrix inversion here, as it also involves moving FEM indices that
involve the overlap matrix. In order to avoid any mistakes, let us derive this directly from the
change of basis of a vector from the symmetry basis to the FEM basis. From equation
\eqref{eq:vector_basis_expansion}, applying an inner product with an FEM basis function, we get
\begin{gather}
    \sum_i f^i \int\dd{\vb{k}} \phi_i(\vb{k})\phi_j(\vb{k})
    = \sum_a \tilde{f}^a \int\dd{\vb{k}}\psi_a(\vb{k})\phi_j(\vb{k}), \\
    \implies \sum_i M_{ji} f^i = \sum_a U^{a*}_j \tilde{f}^a = \sum_a U^\dagger_{ja} \tilde{f}^a, \\
    \implies f^i = \sum_{a,j} \qty(M^{-1})^{ij} U^\dagger_{ja} \tilde{f}^a
\end{gather}
As this change of basis is the inverse of $U$,
\begin{equation}
    (U^{-1})^i_a = \sum_j \qty(M^{-1})^{ij} U^\dagger_{ja}.
\end{equation}
Since the matrix $U$ is $N_{\text{sym}}\times N_{\text{FEM}}$, this matrix inversion is
$N_{\text{sym}}$ sparse inversions, which is still more efficient thatn the $\mathcal{O}(N^2)$
integrals we had to compute in the FEM basis.

\subsection{Symmetry Basis Overlap Matrix}
When using a perfect symmetry basis, the basis functions are orthonormal. However, in real-world
applications, we use a symmetry basis that only approximates the symmetries of the system. This
means that the symmetry basis also has a nontrivial overlap matrix, which we can call $\tilde{M}$.
Now, the change of basis from the FEM basis to the symmetry basis becomes
\begin{align}
    \sum_b\tilde{f}^b\int\dd{\vb{k}}\psi_b(\vb{k})\psi_a^*(\vb{k})
    &= \sum_i f^i \int\dd{\vb{k}}\phi_i(\vb{k}) \psi_a^*(\vb{k}). \\
    \sum_b\tilde{M}_{ab} \tilde{f}^b &= \sum_i U_{ai} f^i
\end{align}
This means that the change of basis matrix from the FEM basis to the symmetry basis is
replaced by
\begin{equation}
    \tilde{U}^i_j = \sum_a \qty(\tilde{M}^{-1})^{ia} U_{aj}.
\end{equation}
We should also be careful with the change of basis of the in-scattering matrix. The positioning of
the symmetry basis indices is not arbitrary anymore. This time, strictly
\begin{equation}
    U_{ai} = \int\dd{\vb{k}}\psi_a^*(\vb{k})\phi_i(\vb{k}),
\end{equation}
and so
\begin{equation}
    S_{ij} = \sum_{a,b} U^*_{ai} \tilde{S}^{ab} U_{bj}
    = \sum_{a,b} U^\dagger_{ia} \tilde{S}^{ab} U_{bj}.
\end{equation}
This means that the only change from the orthonormal case is the positioning of the indices, and
the actual matrix calculations are the same as before. The only subtlety to keep in mind is that
the transformed velocity $\tilde{v}$ should be transformed with $\tilde{U}$ instead of $U$ this
time.

What about the inverse change of basis, from the symmetry basis to the FEM basis? Luckily, our
sloppiness with the indices cancelled each other out, and the expression stays the same.

\subsection{Arbitrary Scattering Kernels}
The method of expanding the scattering kernel in a symmetry basis is effectively a projection into
a low-rank subspace. So instead of using a symmetry-based basis like spherical harmonics, we could
also use any other arbitrary functions as the basis. The only contraints are
\begin{enumerate}
    \item $C(\vb{k}, \vb{k'}) > 0$ everywhere, because it is representing the rate of
        transition between states. A negative value would be inconsistent with the definition.
    \item Keeping detailed balance. For elastic scattering this means
        $C(\vb{k}, \vb{k'}) = C(\vb{k'}, \vb{k})$.
\end{enumerate}
This lets us construct the basis by hand to follow a certain shape.

In some cases, we have a specific scattering kernel, possibly derived from a microscopic model, and
it is not naturally expressed in terms of a low-rank expansion. In these cases, to get a low-rank
approximation of the kernel, we can do a singular value decomposition of the kernel matrix in the
FEM basis, and keep only the largest singular values. Then, using methods like the Lanczos
algorithm, we can compute the largest singular values and the corresponding singular vectors,
without solving the full eigendecomposition problem.

Because of the detailed balance condition, the kernel matrix is symmetric, so the singular value
decomposition is equivalent to an eigendecomposition, and the singular values are just the absolute
values of the eigenvalues. This means that the calculations are exactly the same, since we have the
eigenvectors as the basis functions.

In order for the decomposition to actually provide a performance boost, it needs to be less
expensive than the matrix inversions. However, if we first fully construct the scattering matrix in
the FEM basis and then apply the decomposition, the cost will be $\mathcal{O}(N^3)$, and we have
constructed a dense matrix anyway. So, for this method to make sense, we have to approximate the
matrix with a lower-resolution version. This is achieved by using a lower-resolution triangulation
of the Fermi surface. This way, if we have a mapping from the low-resolution points to the
high-resolution points, we can construct the full scattering matrix from the low-rank decomposition
of the low-resolution matrix. This can be achieved by least-square fitting of a few nearest neighbor
points from the low-resolution triangulation to each point in the high-resolution triangulation.
Then, the mapping gives
\begin{equation}
    C^{\text{high}} = P C^{\text{low}} P^T,
\end{equation}
and then, from the eigendecomposition of the low-resolution matrix, we have
\begin{equation}
    C^{\text{low}} = V \Lambda V^T,
\end{equation}
and so
\begin{equation}
    C^{\text{high}} = (P V) \Lambda (P V)^T.
\end{equation}
By only keeping the largest eigenvalues, we can get a low-rank approximation of the scattering
kernel in the FEM basis, without having to construct the full matrix. Now, our decomposition is
\begin{equation}
    C(\vb{k}, \vb{k'}) = \sum_{i,j} C^{\text{high}}_{ij} \phi_i(\vb{k})\phi_j(\vb{k'}),
\end{equation}
so that the decomposition basis is the same as the FEM basis. By substituting the FEM basis
functions with the symmetry basis functions in equation \eqref{eq:in_scattering_symmetry}, we
can rewrite the in-scattering term as
\begin{equation}
    S_{ij} = \sum_{a,b} M_{ia} \tilde{S}^{ab} M_{bj} = \sum_{a,b} M_{ia}
    \qty(P V S^{\text{eigen}} (P V)^T)^{ab} M_{bj},
\end{equation}
where $S^{\text{eigen}}$ is the $S$ matrix in the basis of the eigenvectors of the low-resolution
scattering kernel. Since the overlap matrix $M$ is symmetric, the full \enquote{transformation}
matrix $U$ becomes
\begin{equation}
    U = M P V.
\end{equation}

What remains is the calculation of the scattering rate. We can assume the representation of the
scattering kernel in terms of the basis is a linear combination of the FEM basis functions. Then,
directly from \eqref{eq:tau_kernel_fem},
\begin{equation}
    \frac{1}{\tau_j} = \sum_{i} \alpha_i C_{ij} =
    \sum_i \frac{\alpha_i}{6} \sum_{a,b} (PV)_{ia} \Lambda (P V)^T_{bj} =
    \sum_{a,b} \qty(\sum_i \frac{1}{6} \alpha_i (PV)_{ia}) \tilde{C}^{ab} (P V)^T_{bj}.
\end{equation}
If we first apply the sum in the parentheses, then we never have to construct any large dense
matrix in the FEM basis.

\subsection{Gaussian Quadrature}
To do the integrations of the basis functions of the symmetry basis over the FEM elements, we can
use the widely utilized Gaussian quadrature method. In this method, one approximates the integral
as a weighted sum of the function evaluated at specific points in the integrations domain.
\begin{equation}
    \int f(x)\dd{x} \approx \sum_i w_i f(x_i)
\end{equation}
The weights $w_i$ and the points $x_i$ are chosen to give exact results for polynomials up to a
certain degree.

For our 3D case, we need to do 2D Gaussian quadrature over traingular elements. In the $(x,y)$
coordinates of the unit right-angled triangle, this is expressed as
\begin{equation}
    \int_T \dd{\vb{k}} f(\vb{k}) \approx A_T \sum_i w_i f(x_i, y_i).
\end{equation}
where $A_T$ is the area of the triangle ($\alpha(i,j,k)$). As the elements we use are linear and
already introduce an error of $\mathcal{O}(h^2)$, where $h$ is the size of the elements, going up
to a quadrature that is exact for polynomials up to degree 2 should be sufficient. For this, the
integrals of the functions
\begin{equation}
    1, \quad x, \quad y, \quad x^2, \quad xy, \quad y^2
\end{equation}
need to be exact. These are the integrals
\begin{empheq}[left=\empheqlbrace]{gather}
    \int_T \dd{x}\dd{y} = \frac{1}{2}, \\
    \int_T \dd{x}\dd{y} x = \int_T \dd{x}\dd{y} y = \frac{1}{6}, \\
    \int_T \dd{x}\dd{y} x^2 = \int_T \dd{x}\dd{y} y^2 = \frac{1}{12}, \\
    \int_T \dd{x}\dd{y} xy = \frac{1}{24}.
\end{empheq}
The symmetry between $x$ and $y$ means that for any point $(x_i, y_i)$ in the quadrature,
there is also a point $(y_i, x_i)$ with the same weight $w_i$. The points and weights must satisfy
these equations:
\begin{empheq}[left=\empheqlbrace]{gather}
    \sum_i w_i = \frac{1}{2}, \label{eq:quadrature_const} \\
    \sum_i w_i x_i = \sum_i w_i y_i = \frac{1}{6}, \label{eq:quadrature_lin} \\
    \sum_i w_i x_i^2 = \sum_i w_i y_i^2 = \frac{1}{12}, \label{eq:quadrature_quad} \\
    \sum_i w_i x_i y_i = \frac{1}{24}. \label{eq:quadrature_cross}
\end{empheq}
The $x$ coordinates and weights of one point can satisfy \eqref{eq:quadrature_lin}
and \eqref{eq:quadrature_quad} for $x$, and the mirror image can satisfy the same for $y$.
Finally, we need an extra weight to satisfy \eqref{eq:quadrature_const}. The last equation
\eqref{eq:quadrature_cross} can be satisfied by the $x$ and $y$ coordinates of the final
point. This means we need 3 points in total. The equations then become
\begin{empheq}[left=\empheqlbrace]{gather}
    w_1 = w_2, \quad x_1 = y_2, \quad y_1 = x_2, \\
    2w_1 + w_3 = \frac{1}{2}, \\
    w_1 x_1 + w_3 x_3 = \frac{1}{6}, \\
    w_1 x_1^2 + w_3 x_3^2 = \frac{1}{12}, \\
    w_3 x_3^2 = \frac{1}{24}.
\end{empheq}
Solving these equations gives two solutions, one with points on the edges and one with points
on the interior of the triangle. The latter probes more of the interior of the triangle, so it
is preferred. This solution is
\begin{empheq}[left=\empheqlbrace]{gather}
    w_1 = w_2 = w_3 = \frac{1}{6}, \\
    x_1 = y_2 = x_3 = y_3 = \frac{1}{6}, \\
    y_1 = x_2 = \frac{2}{3}.
\end{empheq}
Higher order quadratures rarely help with linear elements.

For the change of basis matrix in 3D with linear elements, the integrand is a product of a
triangular coordinate and a symmetry basis function. So, to compute the value of the integrand
in this quadrature, we can reuse the quadrature coordinates. This is even symmetric for each
point $\vb{k}_i$. This makes the quadrature integral a very simple matrix multiplication.

\subsection{Real-Valued Basis Functions}
While you could use complex-valued functions for the symmetry basis, if not handled carefully,
it can severely reduce the performance. Also, by having complex-valued matrices, the memory usage
is doubled. So, since it is better to stick to real-valued basis functions if possible.

Due to the detailed balance condition, the scattering matrix is Hermitian. This means there exists
a real-valued basis for any scattering kernel, and limiting ourselves to real-valued functions does
not reduce the generality of our model.

For example, instead of the usual spherical harmonics, one can use their real-valued counterparts,
defined as
\begin{empheq}[left=\empheqlbrace]{gather}
    Y_{l,\abs{m}}^R(\theta, \phi) = \frac{1}{\sqrt{2}}(
        Y_{l,m}(\theta, \phi) + Y_{l,-m}(\theta, \phi)), \\
    Y_{l,\abs{m}}^I(\theta, \phi) = \frac{1}{\sqrt{2}i}(
        Y_{l,m}(\theta, \phi) - Y_{l,-m}(\theta, \phi)).
\end{empheq}
Instead of complex valued cylindrical harmonics (which are just complex exponentials $e^{im\phi}$),
their real-valued counterparts are just sines and cosines.
...
diagonal and the minor diagonals surrounding it. We say nearly, because there are also extra corner
...
There are many different ways of dividing up the Fermi surface into triangles. We tried any of these
triangulation methods that we could find, and we found that there is negligible difference between
...
the corresponding column contains multiple $1$s, one for each duplicate point. Now, we have
...
\end{equation}
Replacing the vectors with their reduced space versions, we have
\begin{equation}
    AP^T\vb{x'} = P^T\vb{b'} \implies \vb{x'} = \qty(P A P^T)^{-1} \vb{b'}
\end{equation}
Then, the quantity we want is
\begin{equation}
    \sigma = \vb{a}^T\vb{x} = \qty(P^T \vb{a'})^T \qty(P^T \vb{x'})
    = \vb{a'}^T \qty(P A P^T)^{-1} \vb{b'}.
\end{equation}
Therefore, just transforming everything to the reduced space and solving the system there gives
the correct result. It might be tempting to assume this is the same equation as before the
transformation. The key point is that $P$ is not invertible, since it is not a square matrix. So,
\begin{equation}
    \qty(P A P^T)^{-1} \neq P A^{-1} P^T.
\end{equation}
